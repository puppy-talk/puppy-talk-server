...
import com.puppy.talk.websocket.NotificationService;
...
public class WebSocketMessageService implements WebSocketLookUpService {
    ...
    public void processUserMessage(ChatRoomIdentity chatRoomId, UserIdentity userId, String content) {
        ...
        // 사용자 메시지 WebSocket 브로드캐스트
        // 실제 메시지 저장은 별도 서비스에서 처리
        broadcastUserMessage(chatRoomId, userId, content);
    }
    ...
    public void processTypingStatus(ChatRoomIdentity chatRoomId, UserIdentity userId, boolean isTyping) {
        ...
        notificationService.broadcastTypingStatus(typingMessage);
    }
     ...
     * 사용자 메시지 브로드캐스트
     */
    private void broadcastUserMessage(ChatRoomIdentity chatRoomId, UserIdentity userId, String content) {
        try {
            ChatMessage webSocketMessage = ChatMessage.newMessage(
                null, // 메시지 ID는 실제 저장 후 설정됨
                chatRoomId,
                userId,
                SenderType.USER,
                content,
                true // 사용자 메시지는 기본적으로 읽음 처리
            );
            
            notificationService.broadcastMessage(webSocketMessage);
            
            log.debug("Successfully broadcasted user message for chatRoom={}", 
                chatRoomId.id());
    ...
    private void broadcastReadReceipt(ChatRoomIdentity chatRoomId, UserIdentity userId) {
        try {
            ...
            notificationService.broadcastReadReceipt(readReceiptMessage);
